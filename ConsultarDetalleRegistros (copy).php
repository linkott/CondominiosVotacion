...
if ($idmenu == '-- Seleccione --' or $idmenu == ''){ // Si no seleciona nada muestra todos
   ?>
     <script language='javascript'> 
        $("select#Menu").val('1'); // Selecciona Desayuno por Defecto
	 </script>	
   <?php
   $idmenu="1";  //Desayuno    
}
...
if ($TipoConsulta == 1) {
    $Sql="select transaccion.cedula, fechatransaccion, horatransaccion, nombre, apellidos ,descripcion from transaccion
         inner join persona on transaccion.cedula = persona.cedula
...
$tabla ="<fieldset class='ui-corner-all bordeForm'>
		 ...
		 <tr class='encabezado'>
			...
			<td style='color:#B53706;'>Hora Registro</td>
			<td style='color:#B53706;'>Descripcion</td>
		 </tr>";    		
	$TotalGeneral=0;
    $Cont=1;    
